...
for n=1:N
    for j=1:J
    penalty=penalty+p(j)*[(c(n,j)*(l1-n+1)-landa)*(x(m,n,j)+x(m,n,j)^2/2)];
    end
end
Penalty(m)=penalty;
end
...
for m=1:M
  ...
  for n=1:N
    for j=1:J
    SUM=SUM+w(n,j)*p(j)*A1(m,n,j)/(1+exp(-(b(m,n)*x(m,n,j))+d(m,n)));
    end
  end
S(m)=SUM;
penalty=0;
for n=1:N
    for j=1:J
    penalty=penalty+p(j)*[(c(n,j)*(l1-n+1)-landa)*(A1(m,n,j)*x(m,n,j)+(A1(m,n,j)*x(m,n,j))^2/2)];
    end
end
Penalty(m)=penalty;
end
